...
def create_menu():
    ...
    with gui.set_menu(gui.add_submenu(_text('MENU_CONFIG'), icon=RES_FMT.format(consts.RES_SETTINGS))):
        ...
        if consts.FLAG_FLAT_SOURCES:
            gui.add_submenu_radio(item_source, {
                uid: source.NAME for uid, source in srcs.SOURCES.items()}, CURRENT_CONFIG,
                                  consts.CONFIG_ACTIVE_SOURCE, on_click=on_source)
        else:
            ...
            for uid, source in srcs.SOURCES.items():
                ...
                for index in range(len(parts) - 1):
                    ...
                    if uid_ not in sources_:
                        sources_[uid_] = srcs.CATEGORIES[uid_].NAME, {}
                    sources_ = sources_[uid_][1]
                sources_[uid] = source.NAME
            gui.add_submenu_tree_radio(item_source, sources, CURRENT_CONFIG,
                                       consts.CONFIG_ACTIVE_SOURCE, on_click=on_source, icon=RES_FMT.format(consts.RES_SOURCE))
