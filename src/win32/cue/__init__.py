...
from libs.ctyped.lib import iCUESDK_2019
...
def connect(timeout: Optional[int] = None) -> bool:
    with _LOCK:
        _CONNECTED.clear()
        if iCUESDK_2019.CorsairConnect(_session_changed_handler, None) == _SUCCESS:
            _CONNECTED.wait(timeout)
    return _STATE == enum_iCUESDK.CorsairSessionState.Connected


def disconnect() -> bool:
    return iCUESDK_2019.CorsairDisconnect() == _SUCCESS
...
def get_devices(filter: enum_iCUESDK.CorsairDeviceType = enum_iCUESDK.CorsairDeviceType.All,
    ...
    if iCUESDK_2019.CorsairGetDevices(ctyped.byref(ctyped.struct.CorsairDeviceFilter(
