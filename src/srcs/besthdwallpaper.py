...
CATEGORIES = (
    0, 40, 41, 156, 73, 46, 42, 43, 44, 45, 47, 48, 74, 75, 49, 150, 50, 51, 153, 165, 52, 53, 160, 54, 55, 159, 164,
    56, 57, 58, 76, 1, 12, 13, 14, 186, 154, 111, 15, 16, 152, 59, 17, 72, 18, 19, 22, 167, 20, 21, 151, 2, 193, 191,
    189, 190, 23, 187, 155, 24, 77, 78, 79, 71, 192, 143, 142, 3, 136, 25, 137, 138, 139, 140, 141, 26, 4, 129, 130,
    134, 131, 132, 133, 5, 135, 128, 6, 175, 181, 172, 174, 170, 171, 176, 168, 173, 185, 180, 169, 179, 177, 178, 127,
    114, 112, 7, 110, 8, 60, 61, 62, 63, 64, 65, 66, 67, 68, 69, 9, 162, 28, 29, 161, 144, 145, 146, 147, 148, 149, 10,
    30, 31, 32, 33, 34, 184, 183, 182, 157, 35, 188, 36, 37, 109, 115, 116, 117, 118, 119, 120, 121, 166, 122, 158, 123,
    124, 125, 195, 126, 163, 11, 82, 39, 83, 84, 85, 38, 86, 87, 88, 97, 89, 81, 90, 91, 92, 93, 94, 95, 96, 113, 98,
    99, 101, 107, 100, 108, 102, 104, 105, 103, 106, 80, 194)
RESOLUTIONS = '', 'hd-wallpapers', '2k', '4k-uhd', '5k', '6k', '8k-ultra-hd', 'iphone', 'ipad'


def _get_resolution(element: minihtml.Element) -> tuple[int, int]:
    # noinspection PyTypeChecker
    return tuple(map(int, element[0].datas[0].strip().split('x')))


def _key(resolution: tuple[int, int], element: minihtml.Element) -> int:
...
class BestHDWallpaper(Source):
    ...
    def get_image(cls, **params) -> Iterator[Optional[ImageFile]]:
        ...
        url = request.join_url(URL_BASE, f'most-{params.pop(CONFIG_SORT)}-wallpapers')
        if params[CONFIG_CATEGORY] == CATEGORIES[0]:
            del params[CONFIG_CATEGORY]
        else:
            params[CONFIG_CATEGORY] = str(params[CONFIG_CATEGORY])
        if params[CONFIG_RESOLUTION] == RESOLUTIONS[0]:
            del params[CONFIG_RESOLUTION]
        params['isJson'] = 'true'
        page = 1
        while True:
            if not data:
            ...
            if not response_photo:
                ...
                yield
                continue
            option = max(minihtml.loads(response_photo.text).find_all('optgroup'),
                         key=functools.partial(_key, (photo['imWidth'], photo['imHeight'])))
            resolution = _get_resolution(option)
            yield ImageFile(option[0]['data-download'], width=resolution[0],
