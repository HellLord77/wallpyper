...
from .lib import gdi32 as _gdi32
from .lib import user32 as _user32
...
class HSTRING(_type.HSTRING):
    def __del__(self):
        _combase.WindowsDeleteString(self)

    @classmethod
    def from_string(cls, string: str = ''):
        self = cls()
        _combase.WindowsCreateString(string, len(string), _byref(self))
        return self

    def get_string(self) -> str:
        return _combase.WindowsGetStringRawBuffer(self, None)


class HBRUSH(_type.HBRUSH):
    def __del__(self):
        _gdi32.DeleteObject(self)

    @classmethod
    def from_rgb(cls, r: int = 0, g: int = 0, b: int = 0) -> HBRUSH:
        return cls(_gdi32.CreateSolidBrush(_macro.RGB(r, g, b)))

    @classmethod
    def from_color(cls, index: int) -> HBRUSH:
        return cls(_user32.GetSysColorBrush(index))

    def get_rgb(self) -> _Optional[tuple[int, int, int]]:
        brush = _struct.LOGBRUSH()
        if _gdi32.GetObjectW(self, _sizeof(brush), _byref(brush)):
...
class HDC(_type.HDC):
    ...
    def __del__(self):
        _user32.ReleaseDC(self._hwnd, self)
        if self._selected:
            _gdi32.SelectObject(self, self._selected)
            _gdi32.DeleteDC(self)

    @classmethod
    def from_hwnd(cls, hwnd: _Optional[_type.HWND] = None) -> HDC:
        return cls(_user32.GetDC(hwnd))

    @classmethod
    def from_hbitmap(cls, hbitmap: _type.HBITMAP) -> HDC:
        self = cls(_gdi32.CreateCompatibleDC(None))
        self._selected = _gdi32.SelectObject(self, hbitmap.value)
        return self


class HCURSOR(_type.HCURSOR):
    def __del__(self):
        _user32.DestroyCursor(self)

    @classmethod
    def from_idc(cls, idc: int, hinstance: _Optional[_type.HINSTANCE] = None) -> HCURSOR:
        return cls(_user32.LoadCursorW(hinstance, _macro.MAKEINTRESOURCEW(idc)))


class HICON(_type.HICON):
    def __del__(self):
        _user32.DestroyIcon(self)

    @classmethod
    def from_idi(cls, idi: int, hinstance: _Optional[_type.HINSTANCE] = None) -> HICON:
        return cls(_user32.LoadIconW(hinstance, _macro.MAKEINTRESOURCEW(idi)))
...
class HBITMAP(_type.HBITMAP):
    ...
    def __del__(self):
        _gdi32.DeleteObject(self.value)
    ...
    def from_dimension(cls, width: int = 0, height: int = 0, byte: int = 4) -> HBITMAP:
        return cls(_gdi32.CreateBitmap(width, height, 1, byte * 8, None))

    def _fill_dimensions(self):
        bitmap = _struct.BITMAP()
        if not _gdi32.GetObjectW(self, _sizeof(_struct.BITMAP), _byref(bitmap)):
...
class HMENU(_type.HMENU):
    ...
    def __del__(self):
        _user32.DestroyMenu(self)

    @classmethod
    def from_type(cls, popup: bool = True) -> HMENU:
        return cls(_user32.CreatePopupMenu() if popup else _user32.CreateMenu())

    @classmethod
    def from_hwnd(cls, hwnd: _type.HWND, system: bool = False) -> HMENU:
        self = cls(_user32.GetSystemMenu(hwnd, False) if system else _user32.GetMenu(hwnd))
        self._hwnd = hwnd
        return self

    @classmethod
    def from_idm(cls, idm: int, hinstance: _Optional[_type.HINSTANCE] = None) -> HMENU:
        return cls(_user32.LoadMenuW(hinstance, _macro.MAKEINTRESOURCEW(idm)))

    def set_hwnd(self, hwnd: _type.HWND) -> bool:
        self._hwnd = hwnd
        return bool(_user32.SetMenu(hwnd, self))

    def get_item_count(self) -> int:
        return _user32.GetMenuItemCount(self)

    def delete_item(self, id_or_pos: int, by_pos: bool = False) -> bool:
        return bool(_user32.DeleteMenu(self, id_or_pos, _HMENU_MF[by_pos]))

    def remove_item(self, id_or_pos: int, by_pos: bool = False) -> bool:
        return bool(_user32.RemoveMenu(self, id_or_pos, _HMENU_MF[by_pos]))

    def get_item_id(self, pos: int) -> bool:
        return bool(_user32.GetMenuItemID(self, pos))

    def get_item_state(self, id_or_pos: int, by_pos: bool = False) -> int:
        return _user32.GetMenuState(self, id_or_pos, _HMENU_MF[by_pos])

    def get_item_string(self, id_or_pos: int, by_pos: bool = False) -> str:
        sz = _user32.GetMenuStringW(self, id_or_pos, None, 0, _HMENU_MF[by_pos]) + 1
        buff = _type.LPWSTR('\0' * sz)
        _user32.GetMenuStringW(self, id_or_pos, buff, sz, _HMENU_MF[by_pos])
        return buff.value

    def get_item_submenu(self, pos: int) -> HMENU:
        return type(self)(_user32.GetSubMenu(self, pos))

    def enable_item(self, id_or_pos: int, enable: bool = True, gray: bool = True, by_pos: bool = False) -> bool:
        return bool(_user32.EnableMenuItem(self, id_or_pos, _HMENU_MF[by_pos] | (
    ...
    def check_item(self, id_or_pos: int, check: bool = True, by_pos: bool = False) -> bool:
        return bool(_user32.CheckMenuItem(self, id_or_pos, _HMENU_MF[by_pos] | (
            _const.MF_CHECKED if check else _const.MF_UNCHECKED)))

    def set_default_item(self, id_or_pos: int, by_pos: bool = False) -> bool:
        return bool(_user32.SetMenuDefaultItem(self, id_or_pos, by_pos))
    ...
    def set_item_bitmaps(self, id_or_pos: int, checked: _Optional[_type.HBITMAP] = None,
        ...
        return bool(_user32.SetMenuItemBitmaps(self, id_or_pos, _HMENU_MF[by_pos], checked, unchecked))

    def check_radio_item(self, id_or_pos: int, id_or_pos_first: int, id_or_pos_last: int, by_pos: bool = False) -> bool:
        return bool(_user32.CheckMenuRadioItem(
...
class HRGN(_type.HRGN):
    ...
    def from_corners(cls, x1: int, y1: int, x2: int, y2: int) -> HRGN:
        return cls(_gdi32.CreateRectRgn(x1, y1, x2, y2))

    @classmethod
    def from_rect(cls, rect: _struct.RECT) -> HRGN:
        return cls(_gdi32.CreateRectRgnIndirect(_byref(rect)))

    @classmethod
    def from_combination(cls, self: _type.HRGN, other: _type.HRGN, mode: int = _const.RGN_OR) -> HRGN:
        hrgn = cls.from_corners(0, 0, 0, 0)
        _gdi32.CombineRgn(hrgn, self, other, mode)
        return hrgn

    def get_rect(self) -> _struct.RECT:
        rect = _struct.RECT()
        _gdi32.GetRgnBox(self, _byref(rect))
        return rect

    def is_empty(self) -> bool:
        return bool(_user32.IsRectEmpty(_byref(self.get_rect())))

    def is_equal(self, other: _type.HRGN) -> bool:
        return bool(_gdi32.EqualRgn(self, other))


class HWND(_type.HWND):
    def __del__(self):
        _user32.DestroyWindow(self)

    def is_visible(self) -> bool:
        return bool(_user32.IsWindowVisible(self))

    def send_message(self, msg: int, wparam: int = 0, lparam: int = 0, wait: bool = True) -> int:
        return (_user32.SendMessageW if wait else _user32.PostMessageW)(self, msg, wparam, lparam)

    def show(self, cmd: int = _const.SW_SHOW) -> bool:
        return bool(_user32.ShowWindow(self, cmd))

    def update(self) -> bool:
        return bool(_user32.UpdateWindow(self))
